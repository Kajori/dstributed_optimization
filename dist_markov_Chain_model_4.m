...
for j=1:N
    ...
    for row=1:NO_AREA
        ...
        if(row==1 && mal_node_1==5) x_right=x(3*deg+1:4*deg,j);
        elseif(row==1 && mal_node_1~=5) x_right=x(4*deg+1:5*deg,j);
        elseif(row==2 && mal_node_1==1) x_right=x(4*deg+1:5*deg,j);
        elseif(row==2 && mal_node_1~=1) x_right=x(1:deg,j);
        elseif(row==3 && mal_node_1==2) x_right=x(1:deg,j);  
        elseif(row==3 && mal_node_1~=2) x_right=x(deg+1:2*deg,j);  
        elseif(row==4 && mal_node_1==3) x_right=x(deg+1:2*deg,j); 
        elseif(row==4 && mal_node_1~=3) x_right=x(2*deg+1:3*deg,j);
        elseif(row==5 && mal_node_1==4) x_right=x(2*deg+1:3*deg,j); 
        elseif(row==5 && mal_node_1~=4) x_right=x(3*deg+1:4*deg,j);
        end
        
       if(row==1 && mal_node_2==2)      x_left=x(2*deg+1:3*deg,j);
        elseif(row==1 && mal_node_2~=2) x_left=x(deg+1:2*deg,j);
        elseif(row==2 && mal_node_2==3) x_left=x(3*deg+1:4*deg,j);
        elseif(row==2 && mal_node_2~=3) x_left=x(2*deg+1:3*deg,j);
        elseif(row==3 && mal_node_2==4) x_left=x(4*deg+1:5*deg,j); 
        elseif(row==3 && mal_node_2~=4) x_left=x(3*deg+1:4*deg,j);  
        elseif(row==4 && mal_node_2==5) x_left=x(1:deg,j); 
        elseif(row==4 && mal_node_2~=5) x_left=x(4*deg+1:5*deg,j); 
        elseif(row==5 && mal_node_2==1) x_left=x(deg+1:2*deg,j); 
        elseif(row==5 && mal_node_2~=1) x_left=x(1:deg,j);
        end
        ...
        if(row==1)
            dummy_5=dummy_4+(w12_1(:,j) +w15_1(:,j))+rho*(x_left+x_right);
            x((row-1)*deg+1:row*deg,j+1) =dummy_1\dummy_5;
         elseif(row==2)
              dummy_5=dummy_4+(w23_2(:,j) - w12_2(:,j))+rho*(x_left+x_right);
              x((row-1)*deg+1:row*deg,j+1) =dummy_1\dummy_5;
              w12_2(:,j+1)=w12_2(:,j)-rho*(x(1:deg,j+1)-x(deg+1:2*deg,j+1));   % w12 update as 2 is the successor of 1
        elseif(row==3)
             dummy_5=dummy_4+(w34_3(:,j) - w23_3(:,j))+rho*(x_left+x_right);
             x((row-1)*deg+1:row*deg,j+1)=dummy_1\dummy_5;
             w23_3(:,j+1)=w23_3(:,j)-rho*(x(deg+1:2*deg,j+1)-x(2*deg+1:3*deg,j+1));   % w23 update as 2 is the predecessor of 3
       elseif(row==4)
            dummy_5=dummy_4+(w45_4(:,j) - w34_4(:,j))+rho*(x_left+x_right);
            x((row-1)*deg+1:row*deg,j+1)=dummy_1\dummy_5;
            w34_4(:,j+1)=w34_4(:,j)-rho*(x(2*deg+1:3*deg,j+1)-x(3*deg+1:4*deg,j+1));   
       elseif ( row==5)
            dummy_5=dummy_4+((-1)*w15_5(:,j) - w45_5(:,j))+rho*(x_left+x_right);
            x((row-1)*deg+1:row*deg,j+1)=1.04*dummy_1\dummy_5;
...
% filename='fault_no_detection_1per_12_model4.mat';
% x_no_fault_detection=x;
% 
filename='fault_detection_4per_12_model_4.mat';
x_fault_detection=x;
